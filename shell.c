...
int main(){
	...
	while(1)
	{
		printf("prompt>>");
		fflush(stdin);
		if(fgets(command,MAX,stdin)==NULL)		//get input from terminal
			break;
		...
		else if(command[0]=='s' && command[1] == 't' && command[2] == 'a' && command[3]=='r' && command[4] =='t') //starts the background process
		{
			pid_t pid = atoi(command+6);
			kill(pid,SIGCONT);
		}
		else if(command[0]=='s' && command[1] == 't' && command[2] == 'o' && command[3]=='p' ) //stops the background process
		{
			...
			if(command[strlen(command)-2]=='&')
			{
				...
				flag=1;  //states that the command given has an ampercent
			}
			...
			if(flag==1) //if there is '&' in the command i.e. background command
			{
				ret=fork();
				if(ret==0)
				{
					...
					//the process is not brought to the foreground so it is a background process
					execvp(argument[0],argument);
				}
				else
				{
					//in the parent add the job to the list of background jobs
					addtojob(ret);
				}
			}
